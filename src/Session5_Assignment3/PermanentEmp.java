...
public class PermanentEmp extends Employee {
	...
	// Construct will accept the Basic Salary and Leaves available of the Permanent Employee
	...
	// Calculates the leaves taken and based on the available leaves will determine the salary deduction
	
	void calculate_balance_leaves() {
		System.out.println("How many PAID LEAVES you wish to take?");
		take_paid_leave = get.nextInt();

		if (avail_leave(take_paid_leave, 'P')) // Check if the employee can avail PAID LEAVE using the boolean method
												
		{
			paid_leave = paid_leave - take_paid_leave; // Update the pending PAID LEAVE
														 
			take_paid_leave = 0;  // Reset the taken PAID LEAVE and then NO DEDUCTION in salary
		} 
		else
			paid_leave = 0;
		...
		if (avail_leave(take_sick_leave, 'S'))// Check if the employee can avail SICK LEAVES
		{
			...
			take_sick_leave = 0;// Reset the taken SICK LEAVES and then NO DEDUCTION in salary
		} else
			sick_leave = 0;
		...
		if (avail_leave(take_casual_leave, 'C'))// Check if the employee can avail CASUAL LEAVES
		{
			...
			take_casual_leave = 0;// Reset the taken CASUAL LEAVES thus NO DEDUCTION in salary
		} else
			casual_leave = 0;
	}
	...
	boolean avail_leave(int no_of_leaves, char type_of_leave) {
		if (type_of_leave == 'P') {
			if (paid_leave - take_paid_leave >= 0) // Check if leave can be availed
				...
				// As the leave cannot be availed calculate the no of days to be deducted from the salary
  			    take_paid_leave = Math.abs(paid_leave - take_paid_leave);
				return false;
			}

		} else if (type_of_leave == 'S') {
			if (sick_leave - take_sick_leave >= 0)// Check if leave can be availed
				return true;
			else {
	...
	void calculate_salary() {
		...
		// total leaves to be deducted from the salary
		...
		total_sal = basic + hra - pfa;    // Calculate Salary as per the formula provided
		System.out.println("-------------------------------------");
		print_leave_details();
